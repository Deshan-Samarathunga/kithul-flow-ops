...
const PRODUCT_TYPES = ["sap", "treacle", "toddy"] as const;
...
function mapBucketRow(row: any) {
	return {
		...
		createdAt:
			row.created_at instanceof Date ? row.created_at.toISOString() : (row.created_at as string | null),
		updatedAt:
			row.updated_at instanceof Date ? row.updated_at.toISOString() : (row.updated_at as string | null),
		assignedBatchId: row.assigned_batch_id as string | null,
		draft: {
			id: row.draft_id as string,
			date:
				row.draft_date instanceof Date ? row.draft_date.toISOString() : (row.draft_date as string | null),
...
async function fetchProcessingBatch(batchId: string) {
		const batchQuery = `
			...
			LEFT JOIN processing_batch_buckets pbb ON pb.id = pbb.processing_batch_id
	...
	const bucketsQuery = `
		SELECT b.bucket_id
		FROM processing_batch_buckets pbb
		JOIN buckets b ON b.id = pbb.bucket_id
...
router.get("/buckets", auth, requireRole("Processing", "Administrator"), async (req, res) => {
	try {
		const { status, forBatch } = req.query;
		...
		const query = `
			...
			LEFT JOIN processing_batch_buckets pbb ON pbb.bucket_id = b.id
			LEFT JOIN processing_batches pb ON pb.id = pbb.processing_batch_id
			${whereClause}
		...
		const buckets = rows.map(mapBucketRow);
	...
	try {
		...
		try {
			...
			const insertQuery = `
				...
				RETURNING batch_id, batch_number, scheduled_date, product_type, status, created_at, updated_at
			`;
...
router.patch("/batches/:batchId", auth, requireRole("Processing", "Administrator"), async (req, res) => {
	try {
		...
		if (validated.productType) {
			updateClauses.push(`product_type = $${paramIndex}`);
			params.push(validated.productType);
			paramIndex++;
		}
...
router.put(
	...
	async (req, res) => {
		...
		try {
			...
			const batchQuery = "SELECT id FROM processing_batches WHERE batch_id = $1";
			const { rows: batchRows } = await client.query(batchQuery, [batchId]);
			if (batchRows.length === 0) {
				return res.status(404).json({ error: "Batch not found" });
			}
			const batchPk = batchRows[0].id as number;

			await client.query("BEGIN");

			await client.query("DELETE FROM processing_batch_buckets WHERE processing_batch_id = $1", [batchPk]);
			...
			if (validated.bucketIds.length > 0) {
				...
				const bucketPkIds = bucketRows.map((bucket) => bucket.id as number);
				...
				const insertQuery = `
					INSERT INTO processing_batch_buckets (processing_batch_id, bucket_id)
					VALUES ${insertValues}
				`;

...
export async function insertUser(userId: string, passwordHash: string, name: string | null, role: string) {
  const { rows } = await pool.query(
...
export async function insertCenter(centerId: string, centerName: string, location: string, centerAgent: string, contactPhone: string | null) {
  const { rows } = await pool.query(
